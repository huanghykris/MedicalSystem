...
class MedicalGraph:
    def __init__(self):
        # 获取当前脚本文件的绝对路径，并去掉文件名，只保留目录
        cur_dir = os.path.dirname(os.path.abspath(__file__))
        # 将目录与数据文件的相对路径合并
        input_data_path = os.path.join(cur_dir, 'data', 'medical.json')
        output_data_path = os.path.join(cur_dir, 'data', 'medical_1000.json')
...
if __name__ == '__main__':
    ...
    (Drugs, Foods, Checks, Departments, Producers, Symptoms, Diseases, disease_infos,
     ...
     rels_drug_producer, rels_recommanddrug, rels_symptom, rels_acompany, rels_category) = handler.read_nodes()
    handler.create_diseases_nodes(disease_infos)
